...
public class MapGenerator {
    ...
    private static void dfsHallway(LinkedList<Position> posList, TETile[][] world,
                                   Position curPos, boolean[][] visited, Random random) {
        if (posList.isEmpty()) {
            return;
        }
    ...
    private static boolean isDeadEnd(Position curPos, boolean[][] visited) {
        return  (!inRoom(new Position(curPos.x, curPos.y + 2),
                new Room(new Position(4, 4), Game.WIDTH - 10, Game.HEIGHT - 10))
                        || visited[curPos.x][curPos.y + 2])
                && (!inRoom(new Position(curPos.x, curPos.y - 2),
                        new Room(new Position(4, 4), Game.WIDTH - 10, Game.HEIGHT - 10))
                        || visited[curPos.x][curPos.y - 2])
                && (!inRoom(new Position(curPos.x + 2, curPos.y),
                        new Room(new Position(4, 4), Game.WIDTH - 10, Game.HEIGHT - 10))
