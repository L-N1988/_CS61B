        ...
        if (qName.equals("way")) {
            ...
            if (validWay) {
                ...
                for (GraphDB.Node item : way) {
                    ...
                    // get current way
                    oldWay = g.getWay(g.getNodeID(item));
                    // next node
                    GraphDB.Node next = way.get(index);
                    // insert new nodes mutually
                    if (!oldWay.contains(next)) {
                        oldWay.addLast(next);
                        g.addWay(g.getNodeID(item), oldWay);
                        oldWay = g.getWay(g.getNodeID(next));
                        oldWay.addLast(item);
                        g.addWay(g.getNodeID(next), oldWay);
                    }
                    index++;
                }
            }
